...
- Diagram source files (e.g., Mermaid `.mmd`), images (`.png`, `.svg`), and other non-prose companion artifacts that supplement the skill's textual content SHOULD live under an `assets/` subdirectory at the skill root, alongside `references/`.
- The `assets/` subdirectory sits at the skill root alongside `references/`, so diagram sources have a predictable home and do not bloat markdown files.
- Diagram source blocks MUST NOT be inlined into `SKILL.md` or any reference file when the inlined content would (a) substantially bloat the file, (b) force the agent to load the diagram on every visit even when irrelevant, or (c) prevent the diagram from being edited with diagram-aware tooling.
- `SKILL.md` and reference files SHOULD link to companion assets via relative paths (`./assets/<file>.mmd` from the skill root, `../assets/<file>.mmd` from a reference file).
- Asset filenames MUST be kebab-case and SHOULD name the topic concretely (`sign-in-flow.mmd`, not `flow1.mmd` or `diagram.mmd`).
- Companion assets MUST NOT be the sole carrier of a normative rule. The asset is a visualization of rules stated in prose; if the diagram disappears, the prose alone MUST still convey the rule.
...
- MUST NOT embed a 50+ line Mermaid (or other diagram-source) block directly in `SKILL.md` or a reference file when it could live as a `.mmd` file under `assets/` and be linked. Cost: bloats the markdown, forces the agent to load the diagram on every visit, and prevents diagram-aware tooling from editing it.
