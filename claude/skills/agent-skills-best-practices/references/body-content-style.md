...
An author MAY present examples in either of two forms depending on what teaches the rule best:

- **Plain examples** — a `**Example:**` or `**Examples:**` label — when a single illustration (or a set of same-kind illustrations) is enough and a correct-vs-incorrect contrast would add nothing.
- **Good/Bad split** — a `**Good Examples:**` label followed by a `**Bad Examples:**` label — when contrasting a correct pattern against a failure mode is what teaches the rule. When this split is used, Good SHOULD come before Bad so the correct pattern is established first.
...
- MUST format each example under a bold example label as its own block — a blockquote for prose, or a fenced code block with a real language identifier (e.g., `typescript`, `markdown`, `sh`) for code, Markdown, shell commands, or directory trees. MUST NOT use a fenced `text` block for prose examples. Multiple examples MAY appear under a single bold plural label.
